...
export async function generateRss(props) {
  ...
  for (const post of latestPosts) {
    // 自定义的 follow_challenge 字段内容
    const customField = `
      <follow_challenge>
        <feedId>80839869767553024</feedId>
        <userId>65363564758480896</userId>
      </follow_challenge>
    `;
    ...
    feed.addItem({
      ...
      content: `${await createFeedContent(post)}${customField}`, // 在内容中插入自定义字段
  ...
  try {
    ...
    // 在 RSS 的 <item> 标签中插入自定义标签（可选）
    const modifiedRssContent = rssContent.replace(
      /<\/item>/g,
      `
      <follow_challenge>
        <feedId>80839869767553024</feedId>
    ...
    // 写入文件
    fs.writeFileSync('./public/rss/feed.xml', modifiedRssContent);
